/*   start with"    
rm basic_V1block_DNA_reg.x ; gcc -O3 basic_V1block_DNA_reg.c -o basic_V1block_DNA_reg.x -lm ; ./basic_V1block_DNA_reg.x; gnuplot multiplot_flux.gnu ; gv histon_flux.ps & gnuplot multiplot_netflux.gnu ; gv histon_netflux.ps & gnuplot multiplot5.gnu ; gv histon5.ps&   "*/

//   original name: DNAreg_v5expdyn2_Sondemand.c
...
*/
 ...
 Mold=1.0;Hold=50.0;Dold=0.0;totnDNA=0.0; 
...
// printf("seed %ld\n",dd);
 printf("\nbasic DNA coupled model, normal S-phase\n");
...
//  parameter to controll SLBP S
 ...
 for(step=1;step<=limit;step++){ /* start stepwise integration */
  ...
  //  f1=S*Vtcmax/(1+pow((Hold/IY),n1));   /*   for histone reg. model   */
  //  f2=(gambase+gamind*pow(Hold,n2)/(pow(KY,n2)+pow(Hold,n2)))*Mold;   /*   for histone reg. model   */
   f1=S*Vtcmax*pow(Dold,n1)/(pow(KY,n1)+pow(Dold,n1));  /*   for free DNA reg. model   */
   if(current_time>15000) f1=0.0;   //  ActD block during S-phase
   f2=(gambase+gamind/(1.0+pow(Dold/IY,n2)))*Mold;      /*   for free DNA reg. model   */
   f3=k3*Mold;
   f4=delta*Hold;
   f5=V5;
   f6=k6*Hold*Dold;
